...
#通信
udp_receive_packets = 0
udp_request_packets = 0
rtt_list = []
server_time_list = []
...
while True:
    ...
    if len(text) == 0:
        break
    data = pack(udp_request_packets + 1, "2", text)  # 报文数据，bytes类型
    again = 0  #超时重传机会计数
    while again < 3:
        udp_request_packets += 1
        try:
            sendTime = time.time()
...
# 通信反馈
print("receive udp packets：", udp_receive_packets)
if udp_request_packets==0:
    print("packet loss rate：no  udp_request_packets")
    print("max rtt：0", "min rtt：0", "mean rtt：0","stdev rtt：0")
    print("server overall responsiveness：0")
else:
    print("packet loss rate：", 1 - (udp_receive_packets / udp_request_packets))
print("max rtt：", max(rtt_list), "min rtt：", min(rtt_list), "mean rtt：", sum(rtt_list) / len(rtt_list), "stdev rtt：",
      statistics.stdev(rtt_list))
t1 = datetime.datetime.strptime(server_time_list[udp_receive_packets -1], '%H:%M:%S')
